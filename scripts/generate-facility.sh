#!/bin/bash
# 施設画像生成ラッパー
# Usage: bash scripts/generate-facility.sh --name <id> --desc "facility description"
#
# 生成フロー:
#   1. fal.ai で高解像度画像を生成 (seedream v4.5)
#   2. documents/images/facilities/<name>.png に原寸保存
#   3. static/images/facilities/<name>.png に 200x200 リサイズ
#
# Examples:
#   bash scripts/generate-facility.sh --name watermill \
#     --desc "A charming fantasy watermill cottage beside a gentle stream"
#
#   bash scripts/generate-facility.sh --name library \
#     --desc "A cozy fantasy library tower in a small village" --size portrait_4_3

set -e

SKILL_DIR="/home/hirayama/.claude/skills/fal-generate/scripts"
MODEL="fal-ai/bytedance/seedream/v4.5/text-to-image"
STYLE_PROMPT="painterly illustration style with warm earth tones, storybook fantasy art, cozy alchemy village aesthetic, soft lighting"
ORIG_DIR="documents/images/facilities"
STATIC_DIR="static/images/facilities"
THUMB_SIZE=200
SIZE="square_hd"
NAME=""
DESC=""
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --help|-h)
      echo "Usage: bash scripts/generate-facility.sh --name <id> --desc \"facility description\" [options]"
      echo ""
      echo "Options:"
      echo "  --name, -n   Facility ID / output filename (without extension). Required."
      echo "  --desc, -d   Facility description in English. Required."
...
echo "Generating facility image: $NAME" >&2
echo "  Desc: $DESC" >&2
echo "  Model: $MODEL" >&2
...
# プロンプト保存
PROMPT_DIR="documents/image-prompts/facilities"
