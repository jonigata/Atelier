...
bashで$()使うと毎度パーミッション伺いが出るので、他の手段があるときはそちらを使ってください。

## UI規約

### 座標系など

main/documents/technical/responsive-scaling.mdを参照のこと

### z-indexレイヤー規約

`documents/technical/z-index-layers.md` を参照のこと。要点:

- `:root`にCSS変数 `--z-panel`(100), `--z-indicator`(500), `--z-toast`(800), `--z-modal`(1000), `--z-cutscene`(1100), `--z-debug`(9000) を定義済み
- `position: fixed` のオーバーレイには必ずCSS変数を使う（生の数値を使わない）
- コンポーネント内部の重なり（local層）は 0〜10 の直接値でOK
- **禁止**: `position: fixed` のオーバーレイを含む祖先要素に `z-index` を付ける（スタッキングコンテキストの罠）

### ダイアログの操作ヒント

ダイアログやオーバーレイで「タップ/クリックで先に進める」ことを示すには、テキスト（「クリック または Enter で閉じる」等）ではなく、点滅する▼マーカーを使う。モバイル対応のため、操作方法を具体的に書かない。

- 共通コンポーネント: `src/components/common/ContinueMarker.svelte`
- 使い方: `<ContinueMarker />` を配置するだけ
- 「スキップ」ボタン（EventDialog, StudyCompleteDialog等）は別物なのでテキストのまま

## コーディング

本作はゲームであるため、シーケンシャルな手続きが多発します。
その順序は大変重要なので、リアクティブ＋状態遷移などで無理になんとかしようとせず、
なるべく各種コンポーネントを「呼ばれて」「帰る」構造で作成し、
トップレベルでは一つのasync関数で手綱を握るようにしてください。
...
UIのテストについては、なるべくコンポーネント化して、
routes/test/にコンポーネントテスト用のページを作ってください。
コンポーネント用のテストページをdevtoolsでテストするときは別のタブにしてください。

### アニメーション・演出の確認

アニメーションのタイミング確認はブラウザのスクリーンショットでは不可能です。
テストページを作って、ユーザーに実際にブラウザで確認してもらうこと。

### 全スキップ（skipOpening）の更新方法

オープニングの「全スキップ」は `src/lib/data/skipData.json` のセーブデータをロードする仕組み。
フラグやシステムが変わってスキップ後の状態がおかしくなったら、以下の手順で更新する：

1. ゲームを普通にプレイしてスキップ先の正しい状態を作る
2. デバッグパネルのセーブ・ロードでスロット#9に保存
3. 「#9を更新して」と伝える → ブラウザのlocalStorageからJSONを取得して `skipData.json` を上書き
...
### 演出スキップ（skipPresentation）

HUD右上の「演出スキップ」チェックボックスで、全ての演出待ちと単純なクリック待ちをスキップできる。時間短縮・操作縮小が目的。

**制御方法：**
- **チェックボックス**: HUD右上の「演出スキップ」をON/OFF
- **URLパラメータ**: `?skipPresentation=true` でゲーム開始時から自動的にON

**スキップされる演出：**
- DayTransition（日付演出）：瞬時に完了
- EventDialog（イベントダイアログ）：自動的に閉じる

**スキップ時の情報保持：**
- スキップされたイベントの報酬情報はメッセージログに出力される
- 例：「報酬獲得: 所持品解放 / 休息解放」

## 画像

画像を作れと言われたらまず [`documents/technical/image-prompts.md`](documents/technical/image-prompts.md) を読んでください。
環境変数はsetting.jsonに書いてあります。

## コミット

勝手にコミットはしないでください。コミットしろと言われたときだけコミットしてください。
勝手にamendもしないでください。amendしろと言われたときだけamendしてください。
MEMO.md/TODO.md/CLAUDE.mdはいかなる時も一緒にコミットしてよいです
...
- `documents/images/characters/<name>/` — キャラクター画像（立ち絵・シート・表情）

## テストプレイ

テストプレイ(例:15日まで)を命じられたときは、何も考えずに演出スキップでリロードして、プレイヤー目線で15日までプレイし、感想を提出してください。デバッグパネルは使わないでください。15日まで必ずプレイしてください。「クリックがたくさんいる」とか「通知がうざい」とかは人間は無視できるので挙げなくていいです。

あなたの目標は「とにかく15日目にすること」**ではない**ので、必要のない調合や休息で無駄に日数を勧めるようなことは避けてください。休息は、体力回復のためだけに使ってください。使い道のわからない調合はやめてください。
