<script lang="ts">
  ...
  function deliverQuest(quest: ActiveQuest) {
    ...
    const baseReputation = quest.rewardReputation + repBonus + qualityBonus.reputation;
    const finalReputation = Math.floor(baseReputation * (1 + getBuildingReputationExpBonus() + getHelperReputationExpBonus()));
